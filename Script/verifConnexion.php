...
if(isset($_POST['connexion'])) { // si le bouton "Connexion" est appuyé
    ...
    if(empty($_POST['email'])) {
        ...
        if(empty($_POST['mdp'])) {
            ...
            $mysqli = mysqli_connect("localhost", "root", "root", "helpcenter");
            //on vérifie que la connexion s'effectue correctement:
            if(!$mysqli){
                echo "Erreur de connexion à la base de données.";
            } else {
                // on fait maintenant la requête dans la base de données pour rechercher si ces données existe et correspondent:
                $Requete = mysqli_query($mysqli,"SELECT * FROM etudiant WHERE email = '".$email."' AND mdp = '".md5($MotDePasse)."'");//si vous avez enregistré le mot de passe en md5() il vous suffira de faire la vérification en mettant mdp = '".md5($MotDePasse)."' au lieu de mdp = '".$MotDePasse."'
